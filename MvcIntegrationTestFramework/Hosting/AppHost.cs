...
namespace MvcIntegrationTestFramework.Hosting
{
    ...
    public class AppHost
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static AppHost Simulate(string mvcProjectDirectory, bool overwriteAll = false)
        {
            ...
            CopyDllFiles(mvcProjectPath, overwriteAll);
            return new AppHost(mvcProjectPath);
        }

        private static void CopyDllFiles(string mvcProjectPath, bool overwriteAll=false)
        {
            ...
            foreach (var file in Directory.GetFiles(baseDirectory, "*.dll"))
            {
                ...
                if (overwriteAll || !File.Exists(destFile) || File.GetCreationTimeUtc(destFile) != File.GetCreationTimeUtc(file))
                {
